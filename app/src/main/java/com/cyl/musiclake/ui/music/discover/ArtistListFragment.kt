...
class ArtistListFragment : BaseFragment<ArtistListPresenter>(), ArtistListContract.View {
    private val changeListener = object : SingerCateChangeListener {
        override fun change() {
            getArtistList()
        }
    }
    ...
    //适配器
    val areaListAdapter = ArtistCateAdapter(areaList, changeListener)
    val sexListAdapter = ArtistCateAdapter(sexList, changeListener)
    val genreListAdapter = ArtistCateAdapter(genreList, changeListener)
    val indexListAdapter = ArtistCateAdapter(indexList, changeListener)
...
//        val indexListAdapter = ArtistCateAdapter(indexList)
    ...
    fun getArtistList() {
        val area = areaListAdapter.selectID
        val sex = sexListAdapter.selectID
        val genre = genreListAdapter.selectID
        val index = indexListAdapter.selectID
